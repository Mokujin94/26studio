...
const ReplyComment = observer((props) => {
    ...
    useEffect(() => {
        props.likes.filter(item => {
            if (item.userId === user.user.id && item.status && item.commentId === props.replyId) {
                setIsLike(true)
            }
        })
    }, [props.likes])
    ...
    return (
        <div className={style.block} >
            <div className={style.block__text}>
                ...
                <span ref={blockRef} className={commentIsExpand ? style.block__textComment : style.block__textComment + " " + style.block__textComment_expand}> {props.userReply && <span><Link to={PROFILE_ROUTE + '/' + props.userReply.id}>@{props.userReply.name}</Link></span>} {props.comment}</span>
