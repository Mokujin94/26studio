...
function ProjectContent({ pathFromProject, baseURL }) {
	...
	const [activeTab, setActiveTab] = useState("preview"); // "preview" или "code"
		...
		<div className={style.wrapper}>
			...
			<div className={style.tabsContainer}>
				<div className={style.tabs}>
					<button
						className={`${style.tab} ${activeTab === "preview" ? style.active : ""}`}
						onClick={() => setActiveTab("preview")}
					>
						<svg viewBox="0 0 24 24" fill="none" className={style.tabIcon}>
					...
					<button
						className={`${style.tab} ${activeTab === "code" ? style.active : ""}`}
			...
			{activeTab === "preview" ? (
				...
				<div className={style.codeContainer}>
					<CodeViewer projectPath={pathFromProject} />
				</div>
			)}
		</div>
	);
