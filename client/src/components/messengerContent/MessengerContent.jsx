...
const MessengerContent = observer(
	({
		...
		const firstUnreadId = React.useMemo(() => {
			...
			const sorted = [...notReadMessages].sort(
		...
		useEffect(() => {
			const lastOnline =
				new Date(chatData?.member?.lastOnline).getTime() / 1000;
		...
		useEffect(() => {
			...
			if (!hash) return;
			fetchPersonalChat(Number(hash), user.user.id)
				.then((data) => {
					if (!data.is_chat) {
			...
			messages.map((group) => {
				return group.map(async (message) => {
					if (message.id === messageId && message.userId !== user.user.id) {
						...
						message.isRead = true;
						user.socket.emit("onReadMessage", {
							message: message,
							recipientId: hash,
						}); // Объединяем старое и новое сообщение
						user.socket.emit("onNotReadMessage", {
							message: message,
		...
		const handleScrollToBottom = useCallback(() => {
			if (windowChat.current) {
				const scrollBottom =
					windowChat.current.scrollHeight -
					windowChat.current.scrollTop -
					windowChat.current.clientHeight;
		...
		useEffect(() => {
			...
			const isLoaded = messages.some((group) =>
				group.some((msg) => msg.id === firstUnread.id)
			);
						...
						return prevMessages.map((group) => {
							return group.map((oldMessage) => {
								if (oldMessage.id === message.id) {
									const serverFiles = (data.files || []).map(
										(f) => process.env.REACT_APP_API_URL + f
									);
									return {
										...oldMessage,
										load: false,
		...
		const contentOutsideChat = (
			<div className={style.content__inner}>
				<span className={style.content__innerText}>
			...
			<>
				<div className={style.content__header}>
					<div className={style.content__headerInfo}>
						<span className={style.content__headerInfoName}>
							{Number(hash) === user.user.id ? "Избранное" : otherUserData.name}
						</span>
						{isWriting ? (
							<span
								className={
									style.content__headerInfoOnline +
									" " +
									style.content__headerInfoOnline_writing +
									" " +
									style.content__headerInfoOnline_primary
								}
							>
								Печатает
							</span>
				...
				<div
					className={style.content__inner}
				...
				>
					...
					<TransitionGroup component={null}>
						{messages.map((messageGroup, index) => {
							...
							if (
								!lastGroupDate ||
								isDifferentDay(new Date(groupDate), new Date(lastGroupDate))
							) {
								elements.push(
									<div
										key={`date-${groupDate}`}
										className={style.content__inner_date}
									>
							...
							if (
								firstUnreadId &&
								messageGroup.some((m) => m.id === firstUnreadId)
							) {
								elements.push(
									<div
										key={`unread-${firstUnreadId}`}
										className={style.content__inner_unread}
									>
				...
				<div className={style.content__bottom} ref={bottomRef}>
					<div
						className={
							replyMessage.id
								? style.content__bottomReplyWrapper +
								  " " +
								  style.content__bottomReplyWrapper_active
								: style.content__bottomReplyWrapper
						}
					>
						<div className={style.content__bottomReply}>
							...
							<div className={style.content__bottomReplyInfo}>
								<p className={style.content__bottomReplyText}>
									Ответ{" "}
									<span className={style.content__bottomReplyUser}>
										{replyMessage.userName}
									</span>
								</p>
								<span className={style.content__bottomReplyMessage}>
									{replyMessage.text}
								</span>
							</div>
					...
					>
						<div
							className={style.content__innerScrollButton}
						...
						>
							...
							>
								<span
									style={{ minWidth: notReadMessages.length.length * 11 }}
									className={style.content__innerScrollButtonNotReadCount}
								>
		...
		return (
			<div className={style.content}>
				{hash ? contentChat : contentOutsideChat}
			</div>
		);
	}
);
