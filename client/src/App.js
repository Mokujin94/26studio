...
const App = observer(() => {
		...
		if (user.isAuth) {
			const menuAuth = [
				...
				{ id: 3, title: "Группы", icon: group, path: GROUPS_ROUTE },
				{
					id: 4,
					title: "Моя группа",
					groupName: user.user.group.name,
					icon: mygroup,
					path: GROUPS_ROUTE + "/" + user.user.group.id,
				},
	...
	return (
		<SwitchTransition mode="out-in">
			<CSSTransition key={isLoading} timeout={300} classNames="create-anim">
				{isLoading ? (
					<Preloader />
				) : (
					<div className="App">
							...
							<BrowserRouter>
								<AuthProvider>
									...
									>
										<div
											className="modal__bg"
											onMouseDown={() => user.setModalNews(false)}
										>
											<ModalNewsUpload />;
										</div>
									</CSSTransition>
									...
									>
										<div
											className="modal__bg"
											onMouseDown={() => profile.setEditModal(false)}
										>
											<EditModal />;
										</div>
									</CSSTransition>
									...
									>
										<div className="modal__complete">
											<ModalComplete
												completeMessage={modal.modalCompleteMessage}
											/>
										</div>
									</CSSTransition>
									...
									user.path === LOGIN_ROUTE ||
									user.path === PASSWORDRECOVERY_ROUTE ? null : (
										<BurgerMenu />
									)}
									{user.path === REGISTRATION_ROUTE ||
									user.path === LOGIN_ROUTE ||
									user.path ===
										PASSWORDRECOVERY_ROUTE ? null : user.path.includes(
											MESSENGER_ROUTE
									  ) ? (
										<HeaderMessenger />
									) : (
									...
									{user.path === REGISTRATION_ROUTE ||
									user.path === LOGIN_ROUTE ||
									user.path === PASSWORDRECOVERY_ROUTE ||
									user.path.includes(MESSENGER_ROUTE) ? null : (
										<Footer />
									)}
								</AuthProvider>
							</BrowserRouter>
