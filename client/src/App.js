	...
	useEffect(() => {
		if (user.isAuth) {
			const menuAuth = [
				...
				{ id: 4, title: 'Моя группа', groupName: user.user.group.name, icon: mygroup, path: GROUPS_ROUTE + "/" + user.user.group.id },
							...
							{isBrowser
								...
								<BrowserRouter>
									<AuthProvider>
										...
										{user.path === REGISTRATION_ROUTE ||
											user.path === LOGIN_ROUTE || user.path === PASSWORDRECOVERY_ROUTE ? null : user.path.includes(MESSENGER_ROUTE) ?
											<HeaderMessenger /> : (
												<Header />
											)}
										...
										{user.path === REGISTRATION_ROUTE ||
											user.path === LOGIN_ROUTE || user.path === PASSWORDRECOVERY_ROUTE || user.path.includes(MESSENGER_ROUTE) ? null : (
											<Footer />
										)}
									</AuthProvider>
								</BrowserRouter>
